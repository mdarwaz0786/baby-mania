...
const OrderList = () => {
  ...
  const handlePageChange = (newPage) => {
    if (newPage < 1 || (newPage > filters.page && orders?.length === 0)) {
      return;
    }
                ...
                <tbody>
                  {filteredOrders?.map((order, index) => (
                    <tr key={order?._id}>
                      ...
                      <td>
                        <Link to="/admin/order-list" className="text-reset">
                          {order?.products[0]?.product?.skuCode}
                        </Link>
                      </td>
                      ...
                      <td>
                        <div className="dropdown">
                          <button
                            ...
                            type="button"
                            id={`order-context-menu-${order?._id}`}
                          ...
                          <ul
                            ...
                            aria-labelledby={`order-context-menu-${order?._id}`}
                          >
                            <li>
                              <Link
      ...
      <div className="sa-example__body" style={{ marginBottom: "4rem" }}>
        <nav className="d-flex justify-content-between">
          <p>
            ...
            <span>{(filters.page - 1) * filters.limit + orders?.length} </span>
            out of <span>{totalOrder}</span> Orders
          </p>
          ...
          <ul className="pagination pagination-sm">
            ...
            <li
              className={`page-item ${orders?.length === 0 ? "disabled" : ""}`}
            >
              <Link
                to="#"
                onClick={() => handlePageChange(filters.page + 1)}
