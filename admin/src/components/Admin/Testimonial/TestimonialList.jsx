...
const TestimonialList = () => {
  ...
  const deleteTestimonial = async (id) => {
    try {
      await axios.delete(`/api/v1/testimonial/delete-testimonial/${id}`);
      toast.success("testimonial deleted successfully");
      fetchTestimonials();
    } catch (error) {
      console.log("error while deleting testimonial:", error.message);
      toast.error("error while deleting testimonial");
    }
  };

  const updateStatus = async (id, showStatus) => {
    try {
      await axios.put(`/api/v1/testimonial/update-testimonial/${id}`, {
        status: showStatus,
      });
      fetchTestimonials();
    } catch (error) {
